...
public class ClientThread extends Thread {
	...
	public void run() {
		...
		//打印欢迎信息
		pw.println("Connected, sending welcome message...");
		pw.flush();
		...
		while ( b ){
			try {
				...
				pw.flush();
				System.out.println("("+username+") ("+clientIp+")> 331 Failed to get command");
				logger.error(e.getMessage());
				for(StackTraceElement ste : e.getStackTrace()){
					logger.error(ste.toString());
				}
			...
			if(command.toUpperCase().startsWith("USER")){
				System.out.println("(not logged in) ("+clientIp+")> "+command);
				username = command.substring(4).trim(); 
				if("".equals(username)){
					pw.println("501 Syntax error");
					pw.flush();
					System.out.println("(not logged in) ("+clientIp+")> 501 Syntax error");
					username = "not logged in";
				}
				else{
					pw.println("331 Password required for " + username);
					pw.flush();
					System.out.println("(not logged in) ("+clientIp+")> 331 Password required for " + username);
				}
				loginStuts = false;
			} //end USER
			...
			// PASS命令
				...
				if(username.equals("root") && password.equals("root")){
					...
					pw.flush();
					System.out.println("("+username+") ("+clientIp+")> 230 Logged on");
//					System.out.println("客户端 "+clientIp+" 通过 "+username+"用户登录");
					loginStuts = true;
				}
				else{
					pw.println("530 Login or password incorrect!");
					pw.flush();
					System.out.println("(not logged in) ("+clientIp+")> 530 Login or password incorrect!");
					username = "not logged in";
				}
			} //end PASS
			...
			// PWD命令
				...
				if(loginStuts){
//					System.out.println("用户"+clientIp+"："+username+"执行PWD命令");
					//pw.println("257 /""+pdir+"/" is current directory");
					pw.println("257 "+pdir+" is current directory");
					pw.flush();
					//System.out.println("("+username+") ("+clientIp+")> 257 /""+pdir+"/" is current directory");
				}
				else{
					pw.println(LOGIN_WARNING);
					pw.flush();
					System.out.println("("+username+") ("+clientIp+")> "+LOGIN_WARNING);
				}
			} //end PWD
...
//							System.out.println("用户"+clientIp+"："+username+"执行PORT命令");
							pw.println("200 port command successful");
							pw.flush();
							System.out.println("("+username+") ("+clientIp+")> 200 port command successful");
						} catch (ConnectException ce) {
							pw.println("425 Can't open data connection.");
							pw.flush();
							System.out.println("("+username+") ("+clientIp+")> 425 Can't open data connection.");
...
//							System.out.println("用户"+clientIp+"："+username+"执行RETR命令");
...
//						System.out.println("用户"+clientIp+"："+username+"执行LIST命令");
			...
			// 输入非法命令
				...
				pw.flush();
				System.out.println("("+username+") ("+clientIp+")> 500 Syntax error, command unrecognized.");
			}
...
//			System.out.println("用户"+clientIp+"："+username+"退出");
			br.close();
			socketClient.close();
			pw.close();
			if(null != tempsocket){
