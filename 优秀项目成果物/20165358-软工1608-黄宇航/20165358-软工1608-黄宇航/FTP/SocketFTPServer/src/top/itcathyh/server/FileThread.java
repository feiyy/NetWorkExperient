...
public final class FileThread implements Runnable {
    ...
    public FileThread(Socket client, ServerSocket serversocket, String dir,
                      String filename, String cmd, ServerThread st) {
        this.client = client;
            ...
            if (file.isDirectory()) {
                ...
                if (list != null) {
                    for (File tmp : list) {
                        ...
                        if (tmp.isDirectory()) {
                            createSendThread(tmp, dir1 + File.separator + tmp.getName(), length, cot, serversocket);
                        } else {
                            length += tmp.length();
                            tmppos++;
